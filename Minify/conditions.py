    ...
    try:
        if workshop_installed:
            if not os.path.exists(constants.s2v_executable):
                ...
                if helper.download_file(constants.s2v_latest, zip_path, tag):
        ...
        if rg_on_path:
            ...
            if helper.download_file(constants.rg_latest, archive_path, tag):
                helper.add_text_to_terminal("&downloaded_cli_terminal", archive_path)

                success = False
                success = helper.extract_archive(archive_path, ".", f"{archive_name}/{constants.rg_executable}")
        ...
        for folder in constants.mods_with_order:
            ...
            for method_path in workshop_required_methods:
                if os.path.exists(os.path.join(mod_path, method_path)):
                    helper.ui.configure_item(folder, enabled=False, default_value=False)
                    break
